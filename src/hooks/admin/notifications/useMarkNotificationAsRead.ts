...
export const useMarkNotificationAsRead = () => {
  ...
  const { mutate, mutateAsync, isPending } = useMutation({
    mutationFn: async (
      ...
      const response = await authFetch(`/api/notifications/${notification_id}`, {
        method: 'PATCH',
        headers: {
      ...
      const result =
        await getResponseJson<MarkNotificationAsReadResponse>(response);
  ...
  return {
    ...
    markNotificationAsReadAsync: mutateAsync,
    is_marking_notification_as_read: isPending,
  };
};