...
const NotificationTool = ({ className }: NotificationToolProps) => {
  ...
  return (
    <button
      ...
      onClick={handleClick}
      className={cn(
        'relative hidden lg:flex h-10 w-10 items-center justify-center rounded-full border border-white/10 bg-white/5 text-gray-300 transition hover:bg-white/10 hover:text-white',
        className,
      )}
