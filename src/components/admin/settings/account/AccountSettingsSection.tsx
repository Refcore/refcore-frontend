...
} from '@/schema/accountSettings.schema';
import { User } from '@/model/user.model';
import UploadProfilePicture from './UploadProfilePicture';
import { useUpdateProfile } from '@/hooks/admin/account/useUpdateProfile';

type AccountSettingsSectionProps = {
  user: User;
};

const AccountSettingsFormContent = ({ loading }: { loading: boolean }) => {
  const {
...
const AccountSettingsSection = ({ user }: AccountSettingsSectionProps) => {
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <AccountSettingsFormContent loading={loading} />
      </FormShell>
    </div>
  );
};
